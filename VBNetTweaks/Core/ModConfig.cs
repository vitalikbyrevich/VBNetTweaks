    ...
    public static class ModConfig
    {
     ...
     /*   public static ConfigEntry<float> AILODNearDistance { get; private set; }
        ...
        private static void InitServerConfigs(ConfigFile config)
        {
            ...
            CompressionController.Algorithm = config.BindConfig(compressionSection, "Algorithm", CompressionAlgorithm.Zstd, "Алгоритм сжатия: Deflate или Zstd", synced: true);
            ...
            ZDOThrottleDistance = config.BindConfig(zdoSection, "Distance", 300f, "Дистанция для троттлинга ZDO", synced: true);

            var zoneSection = "07 - Zone Owner Settings";
            ZoneOwnerManager.PingThreshold = config.BindConfig(zoneSection, "PingThreshold", 75, "Порог пинга для смены владельца зоны (мс)", synced: true);
